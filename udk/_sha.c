...
//const char *SHA512_version = "SHA-512" "part of OpenSSL 0.9.8 05 Jul 2005";
...
void OPENSSL_cleanse(void *ptr, size_t len)
{
	...
	if ( memchr(ptr, cleanse_ctr, len) )		cleanse_ctr += 63;
}/* OPENSSL_cleanse */
...
int SHA512_Final (unsigned char *md, SHA512_CTX *c)
{
	...
	n++;
	if ( n > (sizeof(c->u) - 16) ){
		memset (p + n, 0, sizeof(c->u) - n), n=0,	sha512_block (c,p,1);
	}
	...
	if ( len >= sizeof(c->u) ){
		...
		if ( (size_t)data % sizeof(c->u.d[0]) != 0 )
			while ( len >= sizeof(c->u) )
				memcpy (p,data,sizeof(c->u)),	sha512_block (c,p,1),	len -= sizeof(c->u),	data += sizeof(c->u);
		else
#endif
			sha512_block (c,data,len/sizeof(c->u)),	data += len,	len %= sizeof(c->u),	data -= len;
	}
