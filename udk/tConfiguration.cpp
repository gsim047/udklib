...
#ifdef __CYGWIN__
	{
		FILE *f = fopen("/proc/self/exename", "rb");
		if ( f ){
			i = 0;
			while ( !feof(f) ){
				int cc = fgetc(f);
...
//			printf("self - size:%d data:[%s]\n", i, buf);
			fclose(f);
			// todo!!!
			name = string(buf, i) + ".exe";
		}
	} 
#else  // !Cygwin == Linux
	{
		i = readlink("/proc/self/exe", buf, 300);
		if ( i > 0 ){
			buf[i] = 0;
			name = string(buf, i);
		}
...
//	printf("fnameext: [%s]\n", ExtractFileNameExt(name).c_str());
	set(cmdlineSection, pathValue, ExtractFilePath(name));
//	printf("path: [%s]\n", ExtractFilePath(name).c_str());
...
std::vector<string> tConfiguration::getArgs() const
{
	return all;
}// tConfiguration::getArgs
