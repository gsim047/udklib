...
string tDbf::getRec(tFILE &fi, unsigned n) const
{
	if ( n >= size() ) return "";
	fi.seek(recaddr(n));
...
tDbfRec *tDbf::loadRec(unsigned n)
{
	if ( n >= size() ) return NULL;
	tFILE fi(fn, "rb");
	if ( !fi.isOpen() )  return NULL;
	return new tDbfRec(*this, getRec(fi, n), n);
}// tDbf::loadRec


bool tDbf::saveRec(tDbfRec *rec, unsigned n)
{
	if ( !rec ) return false;
	if ( n == (unsigned)-1 ) n = rec->num;
	if ( n >= size() ) return false;
	...
	return fi.write(rec->rec());
}// tDbf::loadRec

/*
bool tDbf::add()
{
	link.push_back(new tDbfRec(*this));
	Hi->nrec += 1;
	return true;
}// tDbf::add
*/
...
void tDbf::flush()
{
	...
	Hi->recaddr = sizeof(dbfs_Header) + sizeof(dbfs_RecInfo) * Ri.size() + 1;
	Hi->reclen = 1;
...
tDbfRec::tDbfRec(tDbf &owner, const string &str, unsigned n) :
	dbf(&owner),
	_del(false),
	num(n)
{
